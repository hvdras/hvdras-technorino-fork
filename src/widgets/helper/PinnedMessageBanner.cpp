    ...
    TwitchChannel *channel)
{
    ...
    this->connect(this->unpinButton_, &Button::leftClicked, [this, channel, pin, hasModRights]() {
        ...
        if (action == 1 && hasModRights && !pin->messageId.isEmpty())
        {
            auto account = getApp()->getAccounts()->twitch.getCurrent();
            if (account && !account->isAnon())
            {
                channel->unpinMessageAs(pin->messageId, *account);
            }
        }
        else
        {
    ...
    if (hasModRights)
    {
        ...
        this->connect(this->icon_, &Button::leftClicked, [this, channel, pin]() {
            ...
            menu->addAction("Unpin Message", [channel, pin]() {
                auto account = getApp()->getAccounts()->twitch.getCurrent();
                if (account && !account->isAnon() && !pin->messageId.isEmpty())
                {
                    channel->unpinMessageAs(pin->messageId, *account);
                }
            });
